...
{
	if(m_Wrapped->GetICallback() != this->GetICallback())
		rehlds_syserror("%s: iCallback desync", __func__);
	if(m_Wrapped->GetFlags() != this->GetFlags())
		rehlds_syserror("%s: flags desync", __func__);

	CSteamCallbackCall1 fcall(m_Id, pvParam, m_Size, m_Wrapped);
	CRecorderFuncCall frec(&fcall);
...
void CSteamCallbackRecordingWrapper::Run(void *pvParam, bool bIOFailure, SteamAPICall_t hSteamAPICall)
{
	...
	CSteamCallbackCall2 fcall(m_Id, pvParam, m_Size, bIOFailure, hSteamAPICall, m_Wrapped);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CSteamAppGetCurrentGameLanguageCall fcall;
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetProductCall fcall(pszProduct);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetGameDescCall fcall(pszGameDescription);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetModDirCall fcall(pszModDir);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetDedicatedServerCall fcall(bDedicated);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerLogOnAnonymousCall fcall;
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerBLoggedOnCall fcall;
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerGetSteamIdCall fcall;
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerWasRestartRequestedCall fcall;
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetMaxPlayersCall fcall(cPlayersMax);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetBotCountCall fcall(cBotplayers);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetServerNameCall fcall(pszServerName);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetMapNameCall fcall(pszMapName);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetPasswordProtectedCall fcall(bPasswordProtected);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerClearAllKVsCall fcall;
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetKeyValueCall fcall(pKey, pValue);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSendUserConnectAndAuthenticateCall fcall(unIPClient, pvAuthBlob, cubAuthBlobSize);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerCreateUnauthUserConnectionCall fcall;
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSendUserDisconnectCall fcall(steamIDUser);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerBUpdateUserDataCall fcall(steamIDUser, pchPlayerName, uScore);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerHandleIncomingPacketCall fcall(pData, cbData, srcIP, srcPort);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerGetNextOutgoingPacketCall fcall(cbMaxOut);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerEnableHeartbeatsCall fcall(bActive);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGameServerSetHeartbeatIntervalCall fcall(iHeartbeatInterval);
	CRecorderFuncCall frec(&fcall);
	m_Recorder->PushFunc(&frec);
...
{
	CGetSystemTimeAsFileTimeCall fcall;
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CSocketCall fcall(af, type, protocol);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CIoCtlSocketCall fcall(s, cmd, *argp);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CSetSockOptCall fcall(s, level, optname, optval, optlen);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CRecvFromCall fcall(s, len, flags, *fromlen);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CSendToCall fcall(s, buf, len, flags, to, tolen);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CBindCall fcall(s, addr, namelen);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CGetSockNameCall fcall(s, *namelen);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CGetHostNameCall fcall(namelen);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
void CRecordingEngExtInterceptor::SteamAPI_RegisterCallback(CCallbackBase *pCallback, int iCallback)
{
	...
	if(wrappee->GetFlags() != pCallback->GetFlags())
		rehlds_syserror("%s: flags desync", __func__);
	//if (wrappee->GetICallback() != pCallback->GetICallback()) rehlds_syserror("%s: flags desync", __func__);

	CSteamApiRegisterCallbackCall fcall(wrappee->getRehldsCallbackId(), iCallback, wrappee);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
void CRecordingEngExtInterceptor::SteamAPI_UnregisterCallResult(class CCallbackBase *pCallback, SteamAPICall_t hAPICall)
{
	...
	if(wrappee->GetFlags() != pCallback->GetFlags())
		rehlds_syserror("%s: flags desync", __func__);
	if(wrappee->GetICallback() != pCallback->GetICallback())
		rehlds_syserror("%s: flags desync", __func__);

	CSteamApiUnrigestierCallResultCall fcall(wrappee->getRehldsCallbackId(), hAPICall, wrappee);
	CRecorderFuncCall frec(&fcall);
...
{
	CSteamGameServerInitCall fcall(unIP, usSteamPort, usGamePort, usQueryPort, eServerMode, pchVersionString);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CSteamApiSetBreakpadAppIdCall fcall(unAppID);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CSteamGameServerRunCallbacksCall fcall;
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CSteamApiRunCallbacksCall fcall;
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
{
	CSteamGameServerShutdownCall fcall;
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
...
void CRecordingEngExtInterceptor::SteamAPI_UnregisterCallback(CCallbackBase *pCallback)
{
	...
	CSteamApiUnregisterCallbackCall fcall(wrappee->getRehldsCallbackId(), wrappee);
	CRecorderFuncCall frec(&fcall);
	PushFunc(&frec);
