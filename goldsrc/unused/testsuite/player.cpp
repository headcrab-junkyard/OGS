...
IEngExtCall *CPlayingEngExtInterceptor::getNextCall(bool peek, bool processCallbacks, ExtCallFuncs expectedOpcode, bool needStart, const char *callSource)
{
	...
	{
		if(!cmd->m_Start)
			rehlds_syserror("%s: bad fcall %d; expected start flag", __func__, cmd->getOpcode());
	}
	else
	{
		if(!cmd->m_End)
			rehlds_syserror("%s: bad fcall %d; expected end flag", __func__, cmd->getOpcode());
	}
...
void CPlayingEngExtInterceptor::playSteamCallback1(CSteamCallbackCall1 *cb)
{
	...
	if(itr == m_SteamCallbacks.end())
		rehlds_syserror("%s: callback %d not found", __func__, cb->m_CallbackId);
	CCallbackBase *steamCallback = (*itr).second;

	if(steamCallback->GetFlags() != cb->m_InState.m_nCallbackFlags)
		rehlds_syserror("%s: PRE flags desync", __func__);
	if(steamCallback->GetICallback() != cb->m_InState.m_iCallback)
		rehlds_syserror("%s: PRE flags desync", __func__);
	...
	if(steamCallback->GetFlags() != endCallback->m_OutState.m_nCallbackFlags)
		rehlds_syserror("%s: POST flags desync", __func__);
	if(steamCallback->GetICallback() != endCallback->m_OutState.m_iCallback)
		rehlds_syserror("%s: POST flags desync", __func__);
	freeFuncCall(cb);
...
uint32 CPlayingEngExtInterceptor::time(uint32 *pTime)
{
	...
	freeFuncCall(playCall);
	freeFuncCall(playEndCall);
...
ISteamApps *CPlayingEngExtInterceptor::SteamApps()
{
	...
	if(!playEndCall->m_ReturnNull)
	{
		if(m_SteamAppsWrapper == NULL)
			m_SteamAppsWrapper = new CSteamAppsPlayingWrapper(this);
...
ISteamGameServer *CPlayingEngExtInterceptor::SteamGameServer()
{
	...
	if(!playEndCall->m_ReturnNull)
	{
		if(m_GameServerWrapper == NULL)
			m_GameServerWrapper = new CSteamGameServerPlayingWrapper(this);
...
void CSteamGameServerPlayingWrapper::SetProduct(const char *pszProduct)
{
	...
	m_Player->freeFuncCall(playCall);
	m_Player->freeFuncCall(playEndCall);
}
