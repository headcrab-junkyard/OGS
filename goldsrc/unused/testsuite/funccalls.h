...
public:
	...
	void SetResult(LARGE_INTEGER freq, BOOL res)
	{
...
public:
	...
	void SetResult(LARGE_INTEGER counter, BOOL res)
	{
...
public:
	...
	void setResult(u_long outValue, int res)
	{
		m_Res = res;
		m_OutValue = outValue;
	}
...
public:
	...
	{
		m_Socket = INVALID_SOCKET;
		m_Level = m_OptName = m_OptValLen = 0;
		m_Res = 0;
	}
...
public:
	CSendToCall()
	{
		m_Socket = INVALID_SOCKET, m_Len = m_Flags = m_ToLen = 0;
		m_Res = -1;
	}
...
public:
	CSteamCallbackCall1()
	{
		m_CallbackId = 0;
		m_DataSize = 0;
		m_InState.clear(), m_OutState.clear();
	}
	CSteamCallbackCall1(int cbId, void *data, int dataSize, CCallbackBase *cb);

	void setResult(CCallbackBase *cb)
	{
		m_OutState.m_iCallback = cb->GetICallback();
		m_OutState.m_nCallbackFlags = cb->GetFlags();
	}
	virtual bool compareInputArgs(IEngExtCall *other, bool strict);
	virtual std::string toString();
...
public:
	CSteamCallbackCall2()
	{
		m_CallbackId = m_DataSize = 0;
		m_bIOFailure = false;
		m_SteamAPICall = k_uAPICallInvalid;
		m_InState.clear(), m_OutState.clear();
	}
...
public:
	...
	{
		m_RehldsCallbackId = 0;
		m_iSteamCallbackId = 0;
		m_InState.clear(), m_OutState.clear();
	}
...
public:
	...
	{
		m_IP = 0;
		m_SteamPort = m_GamePort = m_QueryPort = 0;
		m_ServerMode = eServerModeInvalid;
		m_VersionLen = 0;
		m_Res = false;
	}
...
public:
	...
	void setResult(CSteamID &steamId, bool res)
	{
		m_OutSteamId = steamId.ConvertToUint64();
		m_Res = res;
	}
	virtual bool compareInputArgs(IEngExtCall *other, bool strict);
	virtual std::string toString();
