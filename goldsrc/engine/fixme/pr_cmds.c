...
{
	AngleVectors (v, pr_global_struct->v_forward, pr_global_struct->v_right, pr_global_struct->v_up);
}
...
===============
*/
void PF_lightstyle (/*float*/ int style, const char *val)
{
	client_t	*client;
	int			j;
...
=============
*/
float PF_checkbottom (edict_t *ent)
{
...
float PF_cos (float v)
{
	return cos(v);
}

float PF_sqrt (float v)
{
	return sqrt(v);
}
#endif
